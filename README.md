...
**cover 置中裁切**：`scale = max(1000 / 寬, 1000 / 高)`，超出的軸向左右（或上下）等量裁除。不留白、不變形。
...
- **來源照片短邊建議 ≥ 1000px**。小於這個尺寸會被放大而略糊（例如 750px 需放大 33%）。工具不會因此拒絕該檔。
...
```
money-sorry/
├── index.html                    # 全部：HTML + CSS + JS + 內嵌外框（約 53KB）
├── assets/
...
└── openspec/                     # 規格與設計決策
    └── changes/add-money-sorry-frame-tool/
        ├── proposal.md           # 為何做、範圍、Non-goals
        ├── design.md             # 技術決策與理由
        ├── specs/photo-framing/spec.md
        └── tasks.md
```
