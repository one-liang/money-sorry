...
(() => {
   ...
   *
   * 這一層照樣把整個 #explain 解析出來；要哪幾段是 parseSpec 的事。
   */
   ...
   *
   * #detail_detail__item_spec 是 amiami 給規格段落的 id，比日文標題文字穩，
   * 所以優先用它；它缺席時才退回標題比對。兩條路都落空就回空陣列——
   * 「未取得內文」本來就是既有的、看得見的失敗狀態，不必再發明一個。
   */
  function parseSpec(doc) {
    const sections = parseExplain(doc);
    const el = doc.querySelector('#detail_detail__item_spec');
    const text = el ? blockText(el) : '';
    const hit = text ? sections.find(s => s.body === text)
                     : sections.find(s => /仕様/.test(s.heading));
    return hit ? [hit] : [];
  }
  ...
  async function loadProduct(gcode, opts) {
    ...
    return { gcode, title, imageUrls, explain: parseSpec(doc) };
  }

  MS.amiami = {
    jinaUrl, weservUrl, searchUrl, detailUrl,
    fetchDoc, isChallenge, parseSearch, parseProduct, parseExplain, parseSpec, cleanTitle, blockText,
